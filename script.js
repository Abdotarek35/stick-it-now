...
const T = {
  ar: {
    ...
    favCount:'عدد المفضلة', benchLabel:'دكة البدلاء', pitchView:'الملعب', listView:'القائمة', compareLeads:'متفوق في', compareCategories:'فئة'
  },
  en: {
    ...
    favCount:'Favorites count', benchLabel:'Substitutes bench', pitchView:'Pitch', listView:'List', compareLeads:'leads in', compareCategories:'categories'
  }
};
...
const STORAGE_KEYS = { squad:'tacticsboard_squad_v1', favorites:'tacticsboard_favorites_v1', compare:'tacticsboard_compare_v1' };
...
function pitchSlot(p, teamsById, fxMap, badge){
  const team = teamsById[p.team] || {short_name:'—'};
  const fx = fxMap[p.team];
  const oppTeam = fx ? teamsById[fx.opp] : null;
  ...
  return `<div class="pitch-slot">
    <div class="pitch-jersey-badge" style="background:${color}26;border-color:${color}77;">
      ${badge || ''}
...
function renderSquadPitch(squadPlayers, teamsById, fxMap, lang){
  ...
  const captainId = sortedStarters[0]?.id;
  const viceId = sortedStarters[1]?.id;
  const badgeFor = (p) => p.id === captainId ? '<span class="captain-badge">C</span>' : (p.id === viceId ? '<span class="vice-badge">V</span>' : '');

  const gkRow = xi.gk.length ? xi.gk.map(p => pitchSlot(p, teamsById, fxMap, badgeFor(p))).join('') : pitchEmptySlot('GK');
  const defRow = xi.def.map(p => pitchSlot(p, teamsById, fxMap, badgeFor(p))).join('');
  const midRow = xi.mid.map(p => pitchSlot(p, teamsById, fxMap, badgeFor(p))).join('');
  const fwdRow = xi.fwd.map(p => pitchSlot(p, teamsById, fxMap, badgeFor(p))).join('');
  const benchRow = xi.bench.length ? xi.bench.map(p => pitchSlot(p, teamsById, fxMap, '')).join('') : `<p class="lb-empty" style="border:none;">${t.squadSlotEmpty}</p>`;
...
window.addEventListener('DOMContentLoaded', () => {
  moveIndicator(document.getElementById('tabbtn-fantasy'));
